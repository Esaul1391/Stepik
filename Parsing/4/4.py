...
def calculate_total_price(html: str) -> float:
    ...
    all_card = soup.find_all('div', class_="book-card")
    total = 0
    for card in all_card:
        price = float(card.find('p', class_='count price').get_text().split('$')[-1])
        quantity = int(card.find('p', class_='count stock').get_text().split(': ')[-1])
